...
export default function AvatarPreview({avatar=defaultAvatar,className=''}:AvatarPreviewProps){
  ...
  useEffect(()=>{
    ...
    createPreview(host.current,()=>current.current,()=>!cancelled,state=>{if(!cancelled)setStatus(state);})
  ...
  return <div className={`avatar-preview ${className}`} data-avatar-renderer={status}>
...
async function createPreview(host:HTMLElement,getAvatar:()=>AvatarConfig,active:()=>boolean,status:(value:'ready'|'fallback')=>void):Promise<Viewer|undefined>{
  ...
  try{
    ...
    const camera=new T.OrthographicCamera(-2,2,1.6,-1.6,.1,30);camera.position.set(3.3,2.5,6.5);camera.lookAt(0,1.18,0);
    ...
    let frame=0,dead=false,visible=true,target=-.12,ready=false;
    ...
    const update=(avatar:AvatarConfig)=>{
      if(dead)return;
      if(person)disposeAvatar(person);
      person=createAvatar(T,avatar);pivot.add(person);renderer.shadowMap.needsUpdate=true;schedule();
    };
    const resize=()=>{
      ...
      const aspect=width/height,span=Math.max(1.52,.88/aspect);
